 ...
 * </code>
 */
public class OutgoingJsonSecureMarshalFilter extends
        AbstractFilterChainElement<SecureRequest, SecureResponse> {

    private FilterAction next;

    public OutgoingJsonSecureMarshalFilter() {
    ...
    public SecureResponse doFilter(SecureRequest input, Map<String, Object> metadata) throws FilterException {
        ...
        try {
            byte[] jsonString = objectMapper.writeValueAsBytes(input);
            byte[] resultString = (byte[]) next.filter(jsonString, metadata);
